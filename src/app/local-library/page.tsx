...
type SortField = "title" | "artist" | "album" | "genre" | "format" | "bitrate" | "duration";

export default function LocalLibraryPage() {
  const [tracks, setTracks] = useState<LocalTrackRow[]>([]);
  ...
  const fetchTracks = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/local-tracks?${params}`);
      if (res.ok) {
    ...
    <div className="space-y-6 max-w-7xl">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <HardDrive className="w-6 h-6 text-[#3B82F6]" />
          <div>
            <h2 className="text-2xl font-bold text-[#F8FAFC]">Local Library</h2>
            <p className="text-sm text-[#94A3B8]">
              {total.toLocaleString()} tracks scanned
            </p>
          </div>
        </div>
      </div>
                ...
                {tracks.length === 0 && !loading ? (
                  ...
                  tracks.map((track) => (
                    ...
                    >
                      ...
                      <TableCell className="text-[#94A3B8] uppercase text-xs">
                        {track.format ?? "-"}
                      </TableCell>
